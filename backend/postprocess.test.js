// Self-contained test for backend/postprocess.js — no test framework.
// Run: `node backend/postprocess.test.js`. Exits non-zero if any fixture fails.
//
...
console.log("== postprocess() joined-text fixtures ==");
for (const { input, expected } of fixtures) {
  const got = postprocess(input);
  if (got === expected) { passed++; }
  else {
    failed++;
    console.error("FAIL");
    console.error("  in:  ", show(input));
    console.error("  want:", show(expected));
    console.error("  got: ", show(got));
  }
  const twice = postprocess(got);
  if (twice === got) { passed++; }
  else {
    failed++;
    console.error("FAIL (idempotency)");
    console.error("  in:   ", show(input));
    console.error("  once: ", show(got));
    console.error("  twice:", show(twice));
  }
}
