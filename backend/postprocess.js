...
const DICTIONARY_PATH = path.join(
  os.homedir(),
  "Library/Application Support/Voice-to-Text/dictionary.json"
);
...
// echoes (originally from a chunked-transcription era where a splitter could
// overlap a word — kept because models still emit the same echo shape on
// segment boundaries and restarts).
//
...
// incumbents"), the Nemotron punctuator closes a "sentence" on the pause,
// leaving the same word straddling a period or comma. Keep one copy, drop the
// boundary.
//
...
function postprocess(text) {
  ...
  const cleaned = structuralCleanup(lexical);
  if (cleaned === "") return "";
  return cleaned + " ";
}
...
// Keep in sync with the functions above + docs/POSTPROCESS.md.
const RULES = [
