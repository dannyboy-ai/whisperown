...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            with _LK:
                # Chunk long audio (with overlap) instead of one giant pass — a
                # single pass silently drops the tail of long recordings. Files
                # shorter than chunk_duration are a single chunk, so short dictations
                # (the common case) are unaffected.
                text = getattr(
                    M.transcribe(p, chunk_duration=60.0, overlap_duration=15.0),
                    "text", "").strip()
