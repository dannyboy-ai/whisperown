...
`"i like the. the incumbents" → "i like the incumbents"`. The Nemotron/Parakeet
punctuator closes a "sentence" on a think-pause, leaving the restarted word
...
consecutive dictations join cleanly and your cursor lands after `word. ` — never
jammed against the period.
