...
It's a macOS menubar app (one Swift file) + a small Node backend + a local
Parakeet-MLX transcription server. Three moving parts, all on localhost.
...
- **Fast.** Parakeet on Apple Silicon transcribes a typical dictation in well under
  a second (~50 ms for a 4 s clip, ~500 ms for a full minute on an M4 Max).
  Measured, not vibes — see [docs/BENCHMARKS.md](docs/BENCHMARKS.md).
...
- **Honest about cleanup.** A deterministic pipeline strips silence-hallucinations
  ...
  your words with an LLM.** See [docs/POSTPROCESS.md](docs/POSTPROCESS.md).
- **Yours to modify.** One Swift file, a few small JS modules, one Python server.
...
pm2 start server.js --name voice-to-text-backend
```

No config file is needed — with none present, the backend is local-only by default.
...
| **Cleanup Rules…** | View the active post-processing rules (read-only; edit via your agent — see [docs/POSTPROCESS.md](docs/POSTPROCESS.md)). |
| **Parakeet (local · fast)** · **Test Backend** | The backend, and a one-click "is the server up?" check. |
...
- **`backend/postprocess.js`** — the deterministic cleanup pipeline. Every rule is
  documented in plain English in [docs/POSTPROCESS.md](docs/POSTPROCESS.md). Edit by
...
- [docs/BENCHMARKS.md](docs/BENCHMARKS.md) — measured speed across two chips.
- [docs/POSTPROCESS.md](docs/POSTPROCESS.md) — every cleanup rule in plain English.
- [docs/STREAMING-SPEC.md](docs/STREAMING-SPEC.md) — the chunked-streaming design we
  built, measured, and deleted (a warm local model made it moot).
- [docs/MENUBAR-DEBUG.md](docs/MENUBAR-DEBUG.md) — the macOS menubar visibility saga.
...
transcripts, and history stay in `~/Library/Application Support/Voice-to-Text/` on
...
# 2. stop the backend
pm2 delete voice-to-text-backend
...
rm -rf ~/Library/Application\ Support/Voice-to-Text/
```
